...
const client = ipfsHttpClient({
  ...
  protocol: 'https',
  headers: {
    authorization: `Bearer 88c9051cf1f7d7f8fcfd:eedc9b6fea1ce3450c1b1b4129a5582336a11fa89377d4485e40be21fdea0734`
  }
});
export default function Create() {
  const [fileUrl, setFileUrl] = useState(null);
  ...
  const [signer, setSigner] = useState(null);
  const [userAddr, setUserAddr] = useState(null);
  const [hashh, setHashh] = useState(null);
    ...
    if (file) {
      try {
        ...
        const response = await axios({
          ...
          data: formData,
          headers: {
            'pinata_api_key': `88c9051cf1f7d7f8fcfd`,
            'pinata_secret_api_key': `eedc9b6fea1ce3450c1b1b4129a5582336a11fa89377d4485e40be21fdea0734`,
            'Content-Type': 'multipart/form-data'
          }
        });
  
        const ipfsHash = `https://aqua-wise-condor-800.mypinata.cloud/ipfs/${response.data.IpfsHash}`;

        // const hash = ;
        

        setHashh(response.data.IpfsHash);
        ...
        // Update your state with the IPFS hash or perform any other actions with it

      // Update the fileUrl state with the IPFS URL
      setFileUrl(ipfsHash);
  ...
  const onSave = async () => {
    ...
    try {
      const added = await client.add(certificateData);
      const url = `https://aqua-wise-condor-800.mypinata.cloud/ipfs/${hashh}`;
